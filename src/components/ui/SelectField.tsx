...
const SelectField: React.FC<SelectFieldProps> = ({
    ...
    const activeDescendantId =
        highlightedIndex >= 0 ? `${name}-${generatedId}-option-${highlightedIndex}` : undefined;
            ...
            switch (e.key) {
                ...
                default:
                    if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
                        ...
                        const foundOffset = rotated.findIndex((opt) =>
    ...
    return (
        <div className={cn('relative space-y-1', className)} ref={dropdownRef}>
            ...
            >
                <span
                    className={cn('block truncate', !selectedOption && 'text-on-surface-variant')}
                >
                ...
                <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                    <MaterialIcon
                        ...
                        size={24}
                        className={cn(
                            'text-on-surface-variant duration-short4 ease-emphasized transition-transform',
                            isOpen && 'rotate-180',
                        )}
                    />
                </span>
            ...
            {(isOpen || closing) && !disabled && (
                <div
                    className={cn(
                        'bg-surface border-outline shadow-elevation-2 absolute z-50 mt-1 w-full origin-top overflow-hidden rounded-md border py-1',
                        closing
                ...
                >
                    <div className="custom-scrollbar max-h-60 overflow-y-auto py-1" ref={listRef}>
                        {options.length > 0 ? (
                            ...
                            <div className="text-body-small text-on-surface-variant px-4 py-3 text-center italic">
                                Aucune option
                            </div>
                        )}
                    </div>
