...
import { CaretDown, FileCsv, FilePdf, Warning } from '@phosphor-icons/react';
...
const ReportsPage = () => {
    ...
    return (
        <PageContainer>
            ...
            <div className="grid grid-cols-1 medium:grid-cols-2 expanded:grid-cols-2 gap-5">
                ...
                <section className="rounded-lg bg-surface p-4 shadow-elevation-1 flex flex-col justify-between gap-3">
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <b className="font-brand text-[22px] font-semibold tabular-nums text-on-surface">
                                {inventoryRows.length}
                            </b>
                            <span className="text-body-small text-text-secondary">lignes · 11 colonnes</span>
                        </div>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                            {['Réf.', 'Modèle', 'N° de série', 'Statut', 'Détenteur', 'Emplacement', '+5'].map(
                                (col) => (
                                    <code
                                        key={col}
                                        className="rounded-xs bg-surface-container px-1.5 py-0.5 font-mono text-[11px] text-text-secondary"
                                    >
                                        {col}
                                    </code>
                                )
                        ...
                        {canExport && inventoryRows.length > 0 && (
                            <>
                                <Button variant="outlined" size="sm" onClick={() => handleExportCSV('1')}>
                                    <Icon glyph={CaretDown} size={14} /> CSV
                                </Button>
                                <Button variant="outlined" size="sm" onClick={() => handleExportPDF('1')}>
                                    <Icon glyph={CaretDown} size={14} /> PDF
                                </Button>
                            </>
                        )}
                    </div>
                ...
                <section className="rounded-lg bg-surface p-4 shadow-elevation-1 flex flex-col justify-between gap-3">
                    <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <b
                                className={`font-brand text-[22px] font-semibold tabular-nums ${
                                    warrantyRows.length === 0 ? 'text-text-secondary' : 'text-on-surface'
                                }`}
                            >
                                {warrantyRows.length}
                            </b>
                    ...
                    <div className="flex flex-col gap-4">
                        <section className="rounded-lg bg-surface p-4 shadow-elevation-1">
                            <div className="mb-2 flex items-baseline justify-between gap-3">
                                ...
                                <span className="font-brand text-body-small font-semibold tabular-nums text-text-secondary">
                                        ...
                                        {previewSampleRows.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-surface-container/50">
                                                {Object.values(row).map((val, cIdx) => (
                                                    <td key={cIdx} className="px-3 py-2 whitespace-nowrap text-text-secondary">
                                                        {String(val || '—')}
                                                    </td>
                                                ))}
                                            </tr>
                        ...
                        <section className="rounded-lg bg-surface p-4 shadow-elevation-1 flex flex-col gap-3">
                            ...
                            <div className="flex items-center justify-between text-body-small">
                                ...
                                <span className="text-text-secondary">
                                    {activePreview.rows.length} lignes · séparateur virgule
                                </span>
                            </div>
                            ...
                            {canExport && activePreview.rows.length > 0 && (
                                <div className="flex items-center gap-3 pt-2 border-t border-outline-variant">
                                    ...
                                    >
                                        <Icon glyph={FileCsv} size={18} /> Exporter en CSV
                                    </Button>
                                    <Button
                                        variant="tonal"
                                        className="flex-1 justify-center"
                                    ...
                                    >
                                        <Icon glyph={FilePdf} size={18} /> Exporter en PDF
                                    </Button>
                                </div>
                            )}
                        </section>
                        ...
                        {!canExport && (
                            <div className="flex items-start gap-2 rounded-md bg-surface-container p-3 text-body-small text-text-secondary">
