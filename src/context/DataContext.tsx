...
/**
 * Le référentiel géographique — **pays → site → local** (planche 10.1, arbitrage A2).
 *
 * *« L'arbre ne garde que la géographie. Le service en sort, parce que ce n'est pas un
 * lieu : un axe se juge à ce qu'il porte — pays et site portent des actifs, des
 * utilisateurs et une campagne d'audit ; le service, rien. »*
 *
 * Le **local** est le quatrième niveau, facultatif : la salle serveurs est *dans* le
 * Bureau Paris, elle n'est pas un site. Le **service** reste dans la donnée — les
 * campagnes d'audit s'en servent comme périmètre de relevé — mais il n'est plus un
...
interface LocationData {
    ...
    locals: Record<string, string[]>; // Clé: Site -> Valeur: Liste de locaux
...
interface DataContextType {
    ...
    addLocation: (type: LocationKind, name: string, parentId?: string) => boolean;
    renameLocation: (
        type: LocationKind,
        oldName: string,
        newName: string,
    ...
    deleteLocation: (type: LocationKind, name: string, parentId?: string) => void;
...
const STORAGE_KEYS = {
    ...
    locations: { current: 'tracker_locations', legacy: 'neemba_locations' },
} as const;

const normalizeMatch = (value?: string) => (value || '').trim().toLowerCase();

const normalizeMacAddress = (value?: string) => normalizeMatch(value).replace(/[^a-z0-9]/g, '');

const normalizeUuid = (value?: string) => normalizeMatch(value).replace(/[^a-z0-9]/g, '');
...
const resolveEquipmentMatchCandidates = (
...
const mergePersistedUsersWithSeed = (parsed: unknown[]): User[] => {
    ...
    const seedByEmail = new Map(
        mockAllUsersExtended.map((item) => [item.email.toLowerCase(), item]),
    );

    const mergedPersisted = persistedUsers.map((entry) => {
        const seedMatch =
            seedById.get(entry.id) || seedByEmail.get((entry.email || '').toLowerCase());
...
const normalizeEquipmentRecord = (persisted: Equipment, seed?: Equipment): Equipment => {
    ...
    const serialNumber =
        merged.serialNumber ||
        seed?.serialNumber ||
        `SN-${merged.assetId.replace(/[^A-Za-z0-9]/g, '')}`;
    const hostname =
        merged.hostname || seed?.hostname || buildFallbackHostname(merged.name, merged.assetId);
...
const DEFAULT_APPROVAL_IMAGE =
    'https://images.unsplash.com/photo-1517336714731-489689fd1ca4?w=100&h=100&fit=crop';
...
const releaseOrphanedWorkflowEquipment = (
    ...
    const referencedIds = new Set(
        approvalList
            .filter(
                (approval) =>
                    isApprovalActiveStatus(approval.status) && approval.assignedEquipmentId,
            )
...
const mergePersistedRbacWorkflows = (parsed: unknown[]): WorkflowDefinition[] => {
    ...
    const defaultsById = new Map(
        DEFAULT_WORKFLOW_DEFINITIONS.map((workflow) => [workflow.id, workflow]),
    );
...
const mergePersistedRbacAssignments = (
    ...
    const persisted = parsed
        .filter((entry): entry is UserAccessAssignment => {
            if (typeof entry !== 'object' || entry === null) return false;
    ...
    const mergedDefaults = Array.from(defaultsByUserId.values()).map((defaultAssignment) => {
        ...
        return sanitizeAssignment({
            ...
            roleIds: uniqueStrings([
                ...(defaultAssignment.roleIds || []),
                ...(override.roleIds || []),
            ]),
            groupIds: uniqueStrings([
                ...(defaultAssignment.groupIds || []),
                ...(override.groupIds || []),
            ]),
            directPermissions: [
                ...(defaultAssignment.directPermissions || []),
                ...(override.directPermissions || []),
            ],
            temporaryRoles: [
                ...(defaultAssignment.temporaryRoles || []),
                ...(override.temporaryRoles || []),
            ],
            dataScopeOverrides: [
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [settings, setSettings] = useState<AppSettings>(() => {
        try {
            ...
            return saved
                ? { ...DEFAULT_SETTINGS, ...JSON.parse(saved), currency: 'XOF' }
                : DEFAULT_SETTINGS;
        } catch {
    ...
    // --- CATEGORIES ---
    /** La famille d'origine des types du référentiel, pour rattraper une donnée
     *  persistée avant que `Category.family` existe. */
    const SEEDED_CATEGORY_FAMILIES: Record<string, CategoryFamily | undefined> = useMemo(
        () =>
        ...
        try {
            ...
            return parsed.map((cat) => {
                ...
                return {
                    ...
                    /* Même histoire un champ plus loin : une catégorie persistée avant
                       l'ajout de `family` n'en porte pas, et le Catalogue les rangeait
                       toutes sous la même famille de repli — les quatre familles de
                       l'arbitrage A2 devenaient une seule. La famille se **retrouve**
                       ici quand le type est l'un de ceux du référentiel, par son nom :
                       c'est une reprise, pas une supposition. Ce qui reste sans famille
                       le dit à l'écran plutôt que d'être classé d'office. */
                    family:
                        categoryData.family ?? SEEDED_CATEGORY_FAMILIES[categoryData.name || ''],
                    icon:
                        CATEGORY_ICONS[categoryData.iconName || 'Laptop'] ||
    ...
    /** Le référentiel géographique de départ, quand rien n'a encore été enregistré. */
    const SEEDED_LOCATIONS: LocationData = useMemo(
        () => ({
            ...
            /* Le local relevé par la planche : « Salle serveurs » circule déjà dans
               trois planches livrées, dont la feuille de filtre de 04.1. */
            locals: {
                'Bureau Paris': ['Salle serveurs'],
            },
            services: {
    ...
    /**
     * **Le référentiel géographique s'enregistre, comme les huit autres.**
     *
     * Il était le seul état du contexte à ne jamais être écrit : l'initialiseur rendait
     * un objet littéral et aucun effet ne le persistait. Tout pays, site ou service
     * créé — au formulaire comme à l'import — disparaissait au rechargement, sans que
     * rien ne le dise. Un référentiel qu'on ne peut pas alimenter n'est pas un
     * référentiel.
     */
    const [locationData, setLocationData] = useState<LocationData>(() => {
        try {
            ...
            if (!saved) return SEEDED_LOCATIONS;
            ...
            return {
                countries: parsed.countries as string[],
                sites: (parsed.sites ?? {}) as Record<string, string[]>,
                /* Un référentiel enregistré avant l'ajout du local n'en porte pas :
                   il repart des locaux d'origine plutôt que d'un objet vide, sans quoi
                   le seul local relevé disparaîtrait au premier rechargement. */
                locals: (parsed.locals ?? SEEDED_LOCATIONS.locals) as Record<string, string[]>,
                services: (parsed.services ?? {}) as Record<string, string[]>,
            };
        } catch {
            return SEEDED_LOCATIONS;
        }
    ...
    useEffect(() => {
        localStorage.setItem(STORAGE_KEYS.locations.current, JSON.stringify(locationData));
    }, [locationData]);
    ...
    const getRbacAssignmentForUser = useCallback(
        (userId: string): UserAccessAssignment | undefined => {
            return rbacAssignments.find((entry) => entry.userId === userId);
        },
        [rbacAssignments],
    );

    const getEffectiveAccessForUser = useCallback(
        (userId: string): EffectiveAccessProfile | null => {
            const targetUser = users.find((entry) => entry.id === userId);
            if (!targetUser) return null;

            const assignment =
                getRbacAssignmentForUser(userId) ||
                sanitizeAssignment(buildRbacAssignmentFromUser(targetUser));
    ...
    const upsertRbacRole = useCallback(
        (role: RbacRole): BusinessRuleDecision => {
            ...
            logEvent({
                ...
                description: existing
                    ? `Rôle RBAC mis à jour (${normalized.name})`
                    : `Nouveau rôle RBAC créé (${normalized.name})`,
                metadata: {
                    source: 'rbac_roles',
                    roleId: roleId,
                },
    ...
    const deleteRbacRole = useCallback(
        (roleId: string): BusinessRuleDecision => {
            const decision = canManageRbacConfig();
            if (!decision.allowed) return decision;
            ...
            const usedByAssignment = rbacAssignments.some(
                (assignment) =>
    ...
    const upsertRbacGroup = useCallback(
        (group: RbacGroup): BusinessRuleDecision => {
            const decision = canManageRbacConfig();
            if (!decision.allowed) return decision;
            ...
            logEvent({
                ...
                description: existing
                    ? `Groupe RBAC mis à jour (${normalizedGroup.name})`
                    : `Nouveau groupe RBAC créé (${normalizedGroup.name})`,
                metadata: {
                    source: 'rbac_groups',
                    groupId,
                },
    ...
    const deleteRbacGroup = useCallback(
        (groupId: string): BusinessRuleDecision => {
            const decision = canManageRbacConfig();
            if (!decision.allowed) return decision;
            ...
            const assigned = rbacAssignments.some((assignment) =>
                (assignment.groupIds || []).includes(groupId),
            );
            if (assigned) {
                return {
                    allowed: false,
    ...
    const upsertRbacWorkflow = useCallback(
        (workflow: WorkflowDefinition): BusinessRuleDecision => {
            const decision = canManageRbacConfig();
            if (!decision.allowed) return decision;
            ...
            setRbacWorkflows((prev) => {
                if (existing) {
                    return prev.map((entry) =>
                        entry.id === workflowId ? normalizedWorkflow : entry,
                    );
                }
            ...
            logEvent({
                ...
                description: existing
                    ? `Workflow RBAC mis à jour (${normalizedWorkflow.name})`
                    : `Nouveau workflow RBAC créé (${normalizedWorkflow.name})`,
                metadata: {
    ...
    const deleteRbacWorkflow = useCallback(
        (workflowId: string): BusinessRuleDecision => {
            const decision = canManageRbacConfig();
            if (!decision.allowed) return decision;
    ...
    const addUser = useCallback(
        (user: User): BusinessRuleDecision => {
            ...
            const finalUser =
                user.department && serviceManagers[user.department]
    ...
    const updateUser = useCallback(
        (id: string, updates: Partial<User>): BusinessRuleDecision => {
            const permissionDecision = canManageUsersByRole(currentUserAccessRef.current);
            if (!permissionDecision.allowed) {
                return permissionDecision;
            }
    ...
    const deleteUser = useCallback(
        (id: string): BusinessRuleDecision => {
            const permissionDecision = canManageUsersByRole(currentUserAccessRef.current);
            if (!permissionDecision.allowed) {
                return permissionDecision;
            }
            ...
            const hasEquipment = equipment.some(
                (e) => e.user?.id === id || e.user?.name === userToDelete.name,
            );
            const hasActiveApprovals = approvals.some(
                (approval) =>
                    ACTIVE_APPROVAL_STATUSES.includes(approval.status) &&
            ...
            const activeSuperAdminCount = users.filter(
                (existingUser) =>
                    existingUser.role === 'SuperAdmin' && existingUser.status !== 'inactive',
            ).length;
        ...
        (id: string, updates: Partial<Equipment>, logMetadata?: Record<string, unknown>) => {
            ...
            if (oldItem) {
                ...
                if (!oldUserId && nextUserId) {
                    if (
                        nextItem.assignmentStatus === 'CONFIRMED' ||
                        nextItem.status === 'Attribué'
                    ) {
    ...
    const upsertEquipmentFromAuditScan = useCallback(
        (
            ...
            const existing = equipment.find((item) => {
                ...
                if (byHostname && normalizeMatch(item.hostname || item.name) === byHostname)
                    return true;
                return false;
            });
            ...
            const matchedUser = users.find(
                (user) =>
                    ...
                    (scannedUserName &&
                        normalizeMatch(user.name) === normalizeMatch(scannedUserName)),
            );
            const scannedUser =
                scannedUserName || scannedUserEmail
                    ? {
                          id: matchedUser?.id,
                          name:
                              scannedUserName ||
                          ...
                          avatar:
                              matchedUser?.avatar ||
                              buildScanAvatar(scannedUserName || scannedUserEmail || 'user'),
                      }
                    : null;
            ...
            if (existing) {
                ...
                if (payload.machineName && payload.machineName !== existing.name)
                    updates.name = payload.machineName;
                if (payload.assetId && payload.assetId !== existing.assetId)
                    updates.assetId = payload.assetId;
                if (payload.hostname && payload.hostname !== existing.hostname)
                    updates.hostname = payload.hostname;
                if (payload.serialNumber && payload.serialNumber !== existing.serialNumber)
                    updates.serialNumber = payload.serialNumber;
                if (payload.biosUuid && payload.biosUuid !== existing.biosUuid)
                    updates.biosUuid = payload.biosUuid;
                if (payload.macAddress && payload.macAddress !== existing.macAddress)
                    updates.macAddress = payload.macAddress;
                ...
                if (payload.storage && payload.storage !== existing.storage)
                    updates.storage = payload.storage;
            ...
            const generatedAssetId =
                payload.assetId || `ASSET-SCAN-${Date.now().toString().slice(-6)}`;
    ...
    const ingestAgentCheckIn = useCallback(
        (payload: AgentCheckInPayload): AgentCheckInResult => {
            const permissionDecision = canManageInventoryByRole(currentUserAccessRef.current);
            if (!permissionDecision.allowed) {
                return {
                    ok: false,
            ...
            const detectedId =
                existing?.id ||
                `detected_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 6)}`;
            ...
            const candidateEquipmentIds = matchCandidates
                .slice(0, 3)
            ...
            if (canAutoImport) {
                const fallbackCountry =
                    payload.country || linkedEquipment?.country || existing?.country || 'France';
                const targetScope = {
                    country:
                        payload.country ||
                        linkedEquipment?.country ||
                        existing?.country ||
                        'France',
                    site:
                        payload.site ||
                        linkedEquipment?.site ||
                        existing?.site ||
                        DEFAULT_SITE_BY_COUNTRY[fallbackCountry] ||
                        'Bureau Paris',
                    service:
                        payload.service ||
                        linkedEquipment?.department ||
                        existing?.service ||
            ...
            const nextDevice: DetectedDevice = {
                ...
                apps: {
                    ...
                    manageEngine:
                        payload.agents?.manageEngine ?? existing?.apps.manageEngine ?? false,
                },
                status: nextStatus,
                matchConfidence,
        ...
        [
            ...
            equipment,
            logEvent,
            settings.autoCollectionAdEnabled,
            settings.autoCollectionAgentApiKey,
            settings.autoCollectionAgentEnabled,
            settings.autoCollectionNetworkEnabled,
            settings.autoCollectionRequireManualValidation,
            upsertEquipmentFromAuditScan,
        ],
    );
    ...
    const markDetectedDeviceAsIgnored = useCallback(
        (detectedDeviceId: string): boolean => {
            const permissionDecision = canManageInventoryByRole(currentUserAccessRef.current);
            if (!permissionDecision.allowed) return false;
    ...
    const updateApproval = useCallback(
        (
            ...
            if (refusalKind && !reason) {
                return {
                    allowed: false,
                    reason: 'Un motif est requis pour refuser ou renvoyer une demande.',
                };
            }
            ...
            const isDotationRefusal =
                oldApproval.status === 'WAITING_DOTATION_APPROVAL' &&
            ...
            const negativeKind =
                refusalKind ?? (status === 'Cancelled' ? ('CANCEL' as const) : null);
            const decisionNote =
                negativeKind && reason
                    ? {
                          kind: negativeKind,
            ...
            if (oldApproval) {
                ...
                else if (status === 'WAITING_IT_PROCESSING')
                    eventType = isDotationRefusal ? 'APPROVAL_DOTATION_REJECT' : 'APPROVAL_MANAGER';
    ...
    // --- Location Management ---
    const addLocation = useCallback((type: LocationKind, name: string, parentId?: string) => {
        const permissionDecision = canManageLocationsByRole(currentUserAccessRef.current);
        if (!permissionDecision.allowed) {
            return false;
        }
        ...
        setLocationData((prev) => {
            ...
            if (type === 'country') {
                ...
                success = true;
            } else if (type === 'local' && parentId) {
                const currentLocals = newData.locals[parentId] || [];
                if (currentLocals.includes(name)) return prev;
                newData.locals = { ...newData.locals, [parentId]: [...currentLocals, name] };
                success = true;
    ...
    const renameLocation = useCallback(
        (type: LocationKind, oldName: string, newName: string, parentId?: string) => {
            const permissionDecision = canManageLocationsByRole(currentUserAccessRef.current);
            if (!permissionDecision.allowed) {
                return false;
            }
            ...
            setLocationData((prev) => {
                ...
                if (type === 'country') {
                    ...
                    newData.sites[parentId] = currentSites.map((s) =>
                        s === oldName ? newName : s,
                    );
                    /* Locaux et services sont indexés par le nom du site : ils suivent
                   le renommage, sinon ils deviennent orphelins. */
                    if (newData.locals[oldName]) {
                        newData.locals[newName] = newData.locals[oldName];
                        delete newData.locals[oldName];
                    }
                    ...
                    const currentLocals = newData.locals[parentId] || [];
                    if (currentLocals.includes(newName)) return prev;
                    newData.locals[parentId] = currentLocals.map((l) =>
                        l === oldName ? newName : l,
                    );
                    success = true;
                } else if (type === 'service' && parentId) {
                    ...
                    newData.services[parentId] = currentServices.map((s) =>
                        s === oldName ? newName : s,
                    );
    ...
    const deleteLocation = useCallback((type: LocationKind, name: string, parentId?: string) => {
        const permissionDecision = canManageLocationsByRole(currentUserAccessRef.current);
        if (!permissionDecision.allowed) {
            return;
        }
        ...
        setLocationData((prev) => {
            ...
            if (type === 'country') {
                ...
                delete newData.locals[name];
                delete newData.services[name];
            } else if (type === 'local' && parentId) {
                newData.locals[parentId] = (newData.locals[parentId] || []).filter(
                    (l) => l !== name,
                );
            } else if (type === 'service' && parentId) {
                newData.services[parentId] = (newData.services[parentId] || []).filter(
                    (s) => s !== name,
                );
