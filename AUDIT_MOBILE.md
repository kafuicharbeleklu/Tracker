...
- **Onglets — 2 familles :** `PageTabs.tsx` (onglets de page scrollables, ARIA tablist) **et** `SegmentedButton.tsx` (bouton segmenté MD3). Usages distincts mais rôles proches ; à surveiller.
  **TRANCHÉ (Tracker DS v1, 2026-07-25)** — `DESIGN_SYSTEM.md` §11.1 : **documenter, ne pas fusionner**. Les deux rôles ARIA sont incompatibles (`tab` + `aria-selected` + panneau associé vs `group` + `aria-pressed`) ; fusionner imposerait un rôle faux à l'un des deux usages. Règle : la sélection est-elle partageable par URL / remplace-t-elle le contenu principal → `PageTabs` (9 appels, tous des pages) ; est-elle soumise avec le formulaire → `SegmentedButton` (2 appels, tous deux dans une boîte de dialogue).
- **Cartes — 3 primitives :** `Card`, `MetricCard`, `EntityRow` — rôles différents (surface générique / tuile de stat / rangée-entité), pas une vraie duplication mais un continuum à documenter.
  **TRAITÉ (Tracker DS v1)** — continuum documenté en `DESIGN_SYSTEM.md` §11.2, avec l'arbre de décision (collection → `EntityRow` ; un chiffre → `MetricCard` ; sinon `Card`) et les deux pièges (composer une tuile de stat avec `Card` + typo manuelle = l'écart X9 ; bricoler une rangée de liste avec `Card` = perte du repli responsive).
...
- **Tooltip — 2 mécanismes :** le composant maison `<Tooltip>` (3 usages) **vs** l'attribut natif `title=` (**158 occurrences**). Voir §6.
  **TRAITÉ (Tracker DS v1)** — `<Tooltip>` est le composant officiel ; `title=` n'est autorisé qu'en **redondance** d'une information déjà visible ou vocalisée (`DESIGN_SYSTEM.md` §11.3). La règle est outillée en **avertissement** dans `ds:check` : seules les balises natives sont examinées (sur un composant React, `title` est une prop affichée), et les deux redondances mécaniques sont exemptées (`aria-label` sur la même balise ; troncature `truncate`/`line-clamp-*` dont le `title` restitue la version complète). Des 158 occurrences brutes, **1 seule** reste signalée : `DemoBadge`, dont l'infobulle est une définition — consignée au journal du DS.
