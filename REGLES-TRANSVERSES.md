...
### 2.45 · Un rang typographique et une graisse ne cohabitent pas — inscrit le 15/08

Relevé **au portage**, pas à la lecture : les classes de rang du produit
(`.text-body-large`, `.text-title-large`…) déclarent **cinq** propriétés, dont
`font-family` **et** `font-weight`. Posée sur le même élément qu'un utilitaire
`font-brand` ou `font-semibold`, la classe de rang **gagne** — et l'utilitaire
disparaît sans bruit. Les titres de rangée des deux premiers écrans portés sortaient
en **14 / 400** là où la planche dit **15 / 500** ; rien ne le signalait.

**Ce n'est pas un conflit `twMerge`** — les deux classes visent des propriétés que
tailwind-merge ne regroupe pas —, donc la sonde `check-cn-merge` ne peut pas le voir.
C'est la cascade CSS, et elle est muette par construction.

**La règle :** sur un même élément, **ou** un rang typographique, **ou** des
utilitaires de fonte et de graisse. Jamais les deux. Quand la valeur voulue n'est pas
dans l'échelle, on écrit la taille et la graisse à la main — c'est le cas du titre de
rangée, que le registre §2.6 place au rang 3 (14) et que 04.1 dessine à 15 / 500.

**Le contrôle** se fait au rendu : pour chaque élément portant une classe `text-<rang>`,
comparer `getComputedStyle().fontWeight` à la graisse écrite dans le balisage. Une
différence est toujours une perte, jamais une intention.
