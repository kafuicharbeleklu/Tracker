...
> **Exécution (2026-07-22) — (b) validé, pas de second FAB.** **Registre X8 complété** : la réponse système au débordement d'onglets est désormais dans `PageTabs` lui-même — bouton « toutes les vues » (`unfold_more` + compteur, cible 40 px, `aria-haspopup`) en bout de barre, **rendu uniquement quand des onglets débordent** (l'état d'overflow X8 existant pilote l'affordance), ouvrant une `BottomSheet` (primitive DS) listant les vues : libellés longs, icônes, badges, vue active `aria-current` + coche. Activé par défaut pour les 9 consommateurs (`allViewsButton={false}` pour retirer). Vérifié au rendu : RBAC 393 → bouton présent, feuille liste les 4 sections, sélection « Affectations » route `#/rbac/assignments` et ferme ; desktop 1440 → aucun bouton (rien ne déborde). (d) X8-bis en hygiène : **icônes d'onglets masquées en compact** (PageTabs, ~26 px/onglet) + shortLabel « Trouvés » sur l'onglet session Audit — mesuré : les 4 onglets session tiennent presque entiers à 393 (« Écarts » restant à un scroll court, couvert par la feuille).
