...
### D14 (§9.2) — FAIT (`81abdb8`), complété par le recouvrement §9.4-3/5 (`57a2d60`), baselines rafraîchies (`9a53fc9`)

- **Migration du seed, 2 valeurs comme prévu** : id '1' `Pending` → `WAITING_IT_PROCESSING`, id '2' `Approved` → `Completed`. Suppression sèche de tout le reste : les 5 valeurs legacy + `Expired` (et `expiresAt`, jamais écrit/lu) de l'union, lignes legacy d'`APPROVAL_TRANSITIONS`, `LEGACY_APPROVAL_ACTIVE_STATUSES` + helpers `isLegacy`/`isModern`, gates/libellés/actions primaires legacy, `'Approved'` hors `APPROVAL_HISTORY_STATUSES`, toute la mécanique de sections mixtes d'ApprovalsPage (bandeau, « Parcours précédent », `workflowHint` d'ApprovalRow), tests legacy du Dashboard, branches legacy du mapping d'événements (dont la branche `Approved` doublement morte), Set local du Sidebar, entrées legacy de StatusBadge, fallback `'Pending'` du wizard. **−207 lignes.** Les champs legacy d'affichage (`equipmentName`, `requestDate`…) restent en place — hors lot, comme convenu.
- **Recouvrement §9.4-3/5 passé AVANT D13, même logique de fenêtre que D14** : `validationSteps`/`currentStep` (décor jamais avancé/lu — type `ValidationStep` supprimé, seeds et créations nettoyés) et `DISPUTED`/`disputeReason`/`disputedAt`/`ASSIGN_DISPUTED` (mort-né, zéro producteur) auraient été gravés dans `tracker_approvals`/`tracker_equipment` dès la persistance. Le manque fonctionnel « motif de refus » reste consigné à part (voir lots XS).
- **Vérifié au rendu** (smoke 3 personas) : seed '1' s'affiche « Traitement IT » avec bouton Affecter et l'entrée démo du wizard s'ouvre à l'identique ; id '2' « Terminée » dans l'Historique ; le bandeau « parcours précédent » ne peut plus se déclencher ; jane/ethan ne voient rien d'indu. Baselines Approbations rafraîchies une seule fois (3 devices, login = match).
- Piège d'outillage rencontré (consigné en mémoire) : Vite sur hgfs ne voit pas toujours les écritures Python — le serveur dev servait un mockData périmé (seed 'Pending' + règles nouvelles = liste vide) ; un restart du serveur suffit.

### D13 (§9.1) — FAIT (`268e943`), réparation au chargement INCLUSE (restée dans l'effort S)

- **Mécanique exactement comme proposée** : clé `tracker_approvals` (+ legacy `neemba_approvals`), hydratation paresseuse + parse défensif, `mergePersistedApprovalsWithSeed` sur le modèle équipement (garde `id`+`status`, copie persistée prime par id, normalisation `image`/`createdAt`/`updatedAt` — `validationSteps`/`currentStep` n'existent plus, cf. `57a2d60`), `seededMissing` Option B, bypass `DEMO_RESEED_DISABLED` y compris à l'hydratation (`parsed.length > 0 || DEMO_RESEED_DISABLED`), effet de sauvegarde standard. Aucune migration : aucune clé n'a jamais existé.
- **Réparation au chargement incluse** : `releaseOrphanedWorkflowEquipment` (fonction pure) au montage — libère tout équipement `En attente` sur un statut de workflow d'approbation (`WAITING_*`/`PENDING_DELIVERY`, **PENDING_RETURN exclu** : la restitution ne passe pas par une approbation) qu'aucune approbation active ne référence. `setEquipment` fonctionnel, idempotent sous StrictMode. La consigne « vider le storage » de §8.5 est levée.
- **Vérifié au rendu, 6 points** : (1) une demande d'ethan survit au F5 (storage écrit dès la création) ; (2) Option B — le seed '1' avancé par alice (lien ASSET-10001) reste `WAITING_DOTATION_APPROVAL` après reload ; (3) clé supprimée → seed frais (`WAITING_IT_PROCESSING`) ; (4) orphelin fabriqué en storage → libéré `Disponible/NONE/null` au chargement ; (5) contre-épreuve — un lien actif persisté n'est PAS libéré ; (6) bypass : `[]` conservé sous `VITE_DISABLE_DEMO_RESEED=true` (port 3100), re-seedé sans (port 3000).
- Conséquence assumée (consignée §9.1) : la demande seed '1' consommée reste consommée après reload — comportement honnête ; issues = bypass dev ou vidage de la clé. Protocole de capture : `tracker_approvals` s'ajoute aux clés à réinitialiser.

### Lots XS §9.4-1/2/6 + §9.3-C/D — FAIT (`bf69ea7`)

- **§9.4-1 journal du refus** : nouvel événement `APPROVAL_DOTATION_REJECT` (icône `reply`, « a refusé la dotation d'une demande liée à », titre « Dotation refusée »), produit quand `(from, to) = (WAITING_DOTATION_APPROVAL, WAITING_IT_PROCESSING)` — vérifié au rendu : le refus de jane journalise le refus, `metadata.from/to` conservés.
- **§9.4-2 `updatedAt`** : rafraîchi par chaque transition d'`updateApproval` — vérifié (`createdAt` ≠ `updatedAt` après transition, visible dans `tracker_approvals`).
- **§9.4-6 libération complète** : `RELEASED_EQUIPMENT_FIELDS` partagé (refus de dotation + `Rejected`/`Cancelled`), aligné sur la purge du retour — vérifié : l'équipement libéré n'a plus `assignedBy/At/ByName`, `managerValidationBy/At`, `confirmedBy/At`, `reservedFor/At`.
- **§9.3-C couches mortes** : `canValidateRequest`/`canProcessRequest`/`canAssignAsset` (useAccessControl) et `roleCan` (rbacDefaults) supprimés — zéro consommateur, re-vérifié au retrait. Le périmètre pays qu'elles encodaient (seule trace « code » du géo-scoping des mutations d'approbation) disparaît du code : la décision de géo-scoping reste ouverte au registre de §7.5. Effet de bord : `permissions.canManageApprovals` n'a plus aucun consommateur — il tombera avec D16 si « retirer » est confirmé.
- **§9.3-D** : `deleteUser` ouvre désormais sur `canManageUsersByRole`, comme `addUser`/`updateUser` (défense en profondeur alignée ; l'UI masquait déjà le bouton).

### §9.4-4 (annulation par le demandeur) et motif de refus — NON codés, instruction de vigilance confirmée

Les deux sont bien des **fonctionnalités manquantes exigeant leur propre réflexion de garde**, pas des correctifs :

- **Annulation (`Cancelled`)** : le demandeur n'existe nulle part dans le modèle d'autorisation (gates = rôle d'étape + relation manager/bénéficiaire). Brancher « Annuler » exige de trancher : qui annule une demande déléguée (demandeur ? bénéficiaire ?) ; jusqu'à quelle étape (amont seulement — après affectation IT, l'annulation jette le travail IT et doit libérer l'équipement) ; motif capturé ou non ; PIN step-up ; événement dédié (`APPROVAL_CANCEL` n'existe pas). Un chemin ad hoc hors gates recréerait le motif §9.0/§9.3. **Proposé : D17, décision produit.** `Cancelled` reste dans la machine (cible saine, la retirer serait re-churner l'union pour la recréer).
- **Motif de refus** : trois refus distincts (rejet manager, renvoi de dotation, refus de réception), zéro capture depuis le retrait de `validationSteps` (qui ne le capturait pas non plus). À concevoir : champ sur `Approval` + métadonnée d'événement, saisie (dialogue au moment du step-up ?), obligatoire ou non, lisible par qui. **Proposé : D18, avec D17 (même écran, même dialogue probablement).**
